  ...
  if ("Accelerometer" in window || (window.DeviceMotionEvent && isIOS())) {
    ...
    return (
      <div class="bg-zinc-800 w-full min-h-screen flex flex-col gap-4 justify-center items-center text-center">
        <p class="text-slate-300 text-4xl">
          Tvůj prohlížeč nepodporuje akcelerometr! Použij své mobilní zařízení.
        </p>
        <p class="text-slate-300 text-4xl">
          Na androidu prosím použij Google Chrome.
        </p>
      </div>
    );
  }
...
function Game() {
  ...
  const [nextBlockingAvailable, setNextBlockingAvailable] =
    ...
    if (!isIOS()) {
      try {
        ...
        accelerometer.addEventListener("reading", () => {
          if (
            accelerometer.y !== undefined &&
            !isShaking() &&
            accelerometer.y > ANDROID_THRESHOLD
          ) {
            setIsShaking(true);
            fap();
          } else if (
  ...
  const connectSocket = (gameToken: string) => {
    ...
    newSocket.on(
      "user_blocked",
      (data: { blocker: string; blocked: string }) => {
      ...
      {!isAuthenticated() ? (
        ...
        <div class="flex flex-col w-full h-full">
          <details class="w-full p-3 border-b-4 border-zinc-900 text-sm">
            ...
            <button
              class="mt-4 bg-red-500 text-white text-base font-bold px-4 py-2 rounded-xl"
              onclick={() => logout()}
            >
              Odhlásit
            </button>
          ...
          <div class="mx-auto mt-5">
            <p
              class={`text-center text-6xl p-4 ${
                isShaking() &&
                !isBlocked() &&
            ...
            {currentEvent() && (
              <div class="mt-4 p-4 bg-zinc-700 rounded-xl text-center">
                ...
                <p class="mt-2 text-sm text-center">Zbývá: {eventTimeLeft()}</p>
              </div>
            )}
          </div>
          ...
          {isBlocked() && (
            <div class="mb-4 p-3 mx-3 bg-red-900/50 rounded-lg">
              <p class="text-red-200">
                Hráč <span class="font-bold text-lg">{whoBlockedPlayer()}</span>{" "}
                tě zablokoval! Musíš teď chvilku počkat :(
              </p>
            </div>
          )}
              ...
              <tbody>
                {leaderboard().map((user, index) => (
                  ...
                  >
                    ...
                    <td
                      class={`p-2 ${
                        user.name === name() ? "font-bold" : ""
                      } break-words whitespace-normal`}
                    >
                      {user.name}
                    </td>
                    <td class="p-2 text-right sticky right-0 bg-zinc-900">
                      {toText(user.score)}
                    </td>
                  </tr>
                ))}
          ...
          {isBlocked() ? (
            <div>
              <h2 class="font-bold text-center">
                KDYŽ JSI ZABLOKOVANÝ, NENÍ SEXSHOP DOSTUPNÝ
              </h2>
            </div>
          ) : (

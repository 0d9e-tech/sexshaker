...
import {
  ...
  calculateMilenaUpgradeCost,
  calculatePerFapUpgradeCost,
} from "../functions";
...
const ensureUserFields = (user: Partial<User>): User => ({
  ...
  nextBlockingAvailable:
    user.nextBlockingAvailable || defaultUser.nextBlockingAvailable,
...
const initializeStorage = (): {
  ...
  try {
    ...
    const userEntries: [string, User][] = Object.entries(parsed.users).map(
      ([key, value]) => [
        key,
        {
...
io.on("connection", (socket) => {
  ...
  const checkBlockExpiry = () => {
    ...
    if (
      user.isBlocked &&
      user.blockEndTime &&
      now >= new Date(user.blockEndTime)
    ) {
  ...
  socket.on("fap", () => {
    ...
    const scoreIncrease = currentEvent
      ? user.perfap * currentEvent.multiplier
      : user.perfap;
  ...
  socket.on("upgrade_perfap", () => {
    ...
    if (user.score >= upgradeCost) {
      ...
      socket.emit("count", user.score);
      addAuditLog(
        `${user.name} upgraded their PerFap to ${user.perfap}`,
        "SYSTEM"
      );
  ...
  socket.on("upgrade_milena", () => {
    ...
    if (user.score >= upgradeCost) {
      ...
      socket.emit("count", user.score);
      addAuditLog(
        `${user.name} bought a milena (now has ${user.devky})`,
        "SYSTEM"
      );
  ...
  socket.on("upgrade_hentai", () => {
    ...
    if (user.score >= upgradeCost) {
      ...
      socket.emit("count", user.score);
      addAuditLog(
        `${user.name} bought hentai (now has ${user.hentai})`,
        "SYSTEM"
      );
  ...
  socket.on("block_user", (targetUsername: string) => {
    ...
    target.blockEndTime = new Date(
      now.getTime() + blockSettings.blockDuration * 60000
    );

    user.nextBlockingAvailable = new Date(
      now.getTime() + blockSettings.cooldownDuration * 60000
    );

    addAuditLog(
      `${user.name} blocked ${target.name} for ${blockSettings.blockDuration} minutes`,
      "SYSTEM"
    );
  ...
  socket.on("new_user", (username: string) => {
    ...
    if (
      Array.from(users.values()).find((x) => x.name == username) !== undefined
    ) {
      addAuditLog(
        `User ${username} cannot be created (username is already taken)`,
        user.name
      );
      return;
    }
    ...
    addAuditLog(
      `Created new user "${username}" with game token ${newGameToken}`,
      user.name
    );
  ...
  socket.on("user_info", (username: string) => {
    ...
    if (!userEntry) {
      addAuditLog(
        `Failed to fetch user ${username} (user not found)`,
        user.name
      );
      return;
    }
  ...
  socket.on("user_toggle_admin", (username: string) => {
    ...
    socket.emit("user_data", user);
    addAuditLog(
      `Modified ${userToModify.name} admin status: ${userToModify.isAdmin}`,
      user.name
    );
  });
  ...
  socket.on("delete_user", (username: string) => {
    ...
    if (!userEntry) {
      addAuditLog(
        `Failed to delete user ${username} (user not found)`,
        user.name
      );
      return;
    }
    ...
    if (userEntry[1].isAdmin) {
      addAuditLog(
        `Failed to delete user ${username} (user is admin)`,
        user.name
      );
      return;
    }
  ...
  socket.on("rename_user", (oldN: string, newN: string) => {
    ...
    if (Array.from(users.values()).find((x) => x.name == newN) !== undefined) {
      addAuditLog(
        `Failed to rename user ${oldN} to ${newN} (username is already taken)`,
        user.name
      );
      return;
    }
    ...
    if (!userEntry) {
      addAuditLog(
        `Failed to rename user ${oldN} to ${newN} (user not found)`,
        user.name
      );
      return;
    }
  ...
  socket.on(
    "create_event",
    (event: Omit<GameEvent, "eventEnd"> & { eventEnd: string }) => {
      if (!user.isAdmin) return;

      if (currentEvent) {
        addAuditLog(
          `Failed to create event "${event.title}" (another event is already active)`,
          user.name
        );
        return;
      }
      ...
      addAuditLog(
        `Created new event "${event.title}" with ${
          event.multiplier
        }x multiplier, ending at ${currentEvent.eventEnd.toLocaleString()}`,
        user.name
      );
  ...
  socket.on(
    ...
    (event: Omit<GameEvent, "eventEnd"> & { eventEnd: string }) => {
      ...
      addAuditLog(
        `Edited event "${oldTitle}" -> "${event.title}" with ${
          event.multiplier
        }x multiplier, ending at ${currentEvent.eventEnd.toLocaleString()}`,
        user.name
      );
  ...
  socket.on("admin_block_user", (username: string) => {
    ...
    target.blockEndTime = new Date(
      Date.now() + blockSettings.blockDuration * 60000
    );
